...
class MnemonicGeneratorTab(BaseTab):
    ...
    def generate_text(self):
        ...
        password_position = (
            self.password_lines[-1] + self.last_n_phrases + 1
            if self.password_lines
            else 0
        )
    ...
    def insert_character(self, to_replace: list, replace_by: list, check_box_var: bool):
        ...
        if array_criteria:
            replacement_order = (
                (replace_by, to_replace) if check_box_var else (to_replace, replace_by)
            )
            self.replace_characters(
                replacement_order[0], replacement_order[1], not check_box_var
            )
        self.update_text()

    def replace_characters(
        self, insert: list[str], remove: list[str], checkbox_selected: bool = False
    ):
        """
        ...
        for line_index in self.password_lines:
            ...
            character_index = [
                remove.index(each_char)
                for each_char in list(password_line)
                if each_char in remove
            ]
            changed_character = [
                each_char for each_char in list(password_line) if each_char in insert
            ]
            ...
            if changed_character and not checkbox_selected:
                if password_line.index(changed_character[0]) < password_line.index(
                    remove_char
                ):
                    continue
...
class ThemeConverterTab(BaseTab):
    ...
    def convert_theme(self):
        ...
        mnemonic_words = (
            self.base_mnemonic_box.toPlainText().replace("\n", " ").split(" ")
        )
        ...
        try:
            new_mnemonic_words = self.new_mnemonic.convert_theme(
                mnemonic_words, self.new_theme, self.base_theme
            )
            ...
            try:
                ...
                new_mnemonic_words = self.new_mnemonic.convert_theme(
                    try_words, self.new_theme, self.base_theme
                )
                new_mnemonic_words = self.new_mnemonic.format_mnemonic(
                    new_mnemonic_words
                )
                display_text = new_mnemonic_words
            except Exception as error:
                if len(mnemonic_words) % phrase_size == 1 and len(
                    mnemonic_words[0]
                ) == n * len(mnemonic_words[1:]):
...
class TableSelectorTab(BaseTab):
    ...
    def __init__(self, parent: "QtFormosa"):
        ...
        self.selected_indexes = {
            key: self.COLUMN_LINE_PARAGRAPH_SIZE for key in self.states
        }
        ...
        self.panel_widgets = [
            self.reset_button,
            self.sel_valid_phrase_label,
            self.highlight_checkbox,
            self.warning_label,
            self.use_custom_set,
            self.custom_character_entry,
            self.output_button,
        ]
    ...
    def set_tab_layout(self):
        ...
        [
            self.tab_layout.addWidget(
                each_widget, self.panel_widgets.index(each_widget), 0
            )
            for each_widget in self.panel_widgets
        ]

        self.grid_frame_selector.setAlignment(Qt.AlignTop)
        self.tab_layout.addLayout(
            self.grid_frame_selector,
            0,
            2,
    ...
    def init_label_objects(self):
        ...
        for each_syntactic_word in syntactic_word_list:
            ...
            returned_list, returned_bools = self.limit_word_length(
        ...
        [
            font_sizes.update(
                {each_syntactic_word: 7 if row_len_limited[each_syntactic_word] else 8}
            )
            for each_syntactic_word in syntactic_word_list
        ]
                    ...
                    [
                        self.object_dict[each_syntactic_word].append(
                            QLabel(
                                parent=self,
                                text=self._get_text(
                                    limited_list[each_syntactic_word],
                                    column,
                                    line,
                                    paragraph,
                                ),
                        ...
                        for column in range(a)
                        if self._cell_idx(column, line, paragraph)
                        < len(words_dictionary[each_syntactic_word].total_words)
                    ]
                    for line in range(a)
                ]
                for paragraph in range(a)
        ...
        [
            (
                each_object.setFont(QFont("Times", font_sizes[each_syntactic_word])),
                each_object.hide(),
            )
            for each_syntactic_word in syntactic_word_list
    ...
    def limit_word_length(self, wordlist: list) -> (list, bool):
        ...
        row_len_limited = (
            ...
            if any(
                [
                    len(
                        "".join(wordlist[a * each_row_index : (a + 1) * each_row_index])
                    )
                    > a * char_len_limit
        ...
        for each_row_index in range(len(wordlist) // a):
            row_chars += "".join(
                wordlist[a * each_row_index : (a + 1) * each_row_index]
            )
            row_len_limited = (
                True if len(row_chars) > a * char_len_limit else row_len_limited
            )
    ...
    def define_key_list(self):
        ...
        if len(set(characters_list)) == self.COLUMN_LINE_PARAGRAPH_SIZE:
            ...
            self.custom_character_entry.setText(
                self.custom_character_entry.text()[0 : self.COLUMN_LINE_PARAGRAPH_SIZE]
            )
        elif len(set(characters_list)) != len(characters_list):
            # Trim the Entry widget to unique characters
            self.custom_character_entry.setText(
                self.custom_character_entry.text()[0:-1]
            )
    ...
    def randomize_character_set(self):
        ...
        self.input_set_caseless = input_chars + list(
            [each_char.swapcase() for each_char in input_chars]
        )
        column_char_set = input_chars.copy()
        random.shuffle(column_char_set)
        self.input_set_column = column_char_set + list(
            [each_char.swapcase() for each_char in column_char_set]
        )
        line_char_set = input_chars.copy()
        random.shuffle(line_char_set)
        self.input_set_line = line_char_set + list(
            [each_char.swapcase() for each_char in line_char_set]
        )
        paragraph_char_set = input_chars.copy()
        random.shuffle(paragraph_char_set)
        self.input_set_paragraph = paragraph_char_set + list(
            [each_char.swapcase() for each_char in paragraph_char_set]
        )
    ...
    def allocate_grid(self, row_init: int = 1, col_init: int = 1):
        ...
        for paragraph in range(a):
            ...
            for line in range(a):
                ...
                paragraph_widget = self.paragraph_objects[
                    self._line_idx(line, paragraph)
                ]
                self.grid_frame_selector.addWidget(
                    line_widget,
                    self._line_idx(line, paragraph) + row_init,
                    a + 2 * col_init,
                )
                self.grid_frame_selector.addWidget(
                    paragraph_widget, self._line_idx(line, paragraph) + row_init, 0
                )
                ...
                for column in range(a):
                    self.grid_frame_selector.addWidget(
                        self.column_objects[column], 0, col_init + 1 + column
                    )
                    ...
                    if len(current_objects) > self._cell_idx(column, line, paragraph):
                        word_widget = current_objects[
                            self._cell_idx(column, line, paragraph)
                        ]
                        self.grid_frame_selector.addWidget(
                            word_widget,
                            self._line_idx(line, paragraph) + 1,
                            col_init + 1 + column,
                        )
    ...
    def keyReleaseEvent(self, event: "QKeyEvent"):
        ...
        if (
            event.text() in self.input_set_caseless
            and self.current_state != self.states[-1]
        ):
    ...
    def changed_show_highlight(self):
        ...
        if self.highlight_checkbox.isChecked():
            ...
            [
                [
                    each_label.setStyleSheet(self.defaultbg)
                    for each_label in self.object_dict[each_syntactic_word]
                ]
    ...
    def grayout_gen_button(self):
        ...
        state = (len(self.picked_passphrase) % phrase_len == 0) and len(
            self.picked_passphrase
        ) > 0
    ...
    def output_phrases(self):
        ...
        phrase_len = len(self.parent.base_dict.natural_order)
        if (len(self.picked_passphrase) % phrase_len == 0) and len(
            self.picked_passphrase
        ) > 0:
            output_phrases = ""
            ...
            for i in range(len(self.picked_passphrase) // phrase_len):
                phrase = " ".join(
                    self.picked_passphrase[phrase_len * i : phrase_len * (i + 1)]
                )
    ...
    def check_word_list(self, word=None):
        ...
        for filling_order_word in filling_order[1:check_size]:
            if not all(
                value in natural_order[:check_size]
                for value in filling_order[:check_size]
            ):
                continue
            ...
            state = (
                False
                if checklist[natural_index + phrases_shift] not in word_mapping
                else state
            )
        ...
        if self.current_state == self.states[0]:
            ...
            if end + 1 < len(self.object_dict[self.natural_word]):
                self.check_word_list(
                    self.object_dict[self.natural_word][begin : end + 1][0].text()
                )
        color = (
            self.defaultbg if self.current_state == self.states[0] else self.check_color
        )
    ...
    def fill_bg_color(self, color: str, begin: int, end: int, step: int):
        ...
        if self.highlight_checkbox.isChecked():
            ...
            [
                each_label.setStyleSheet(self.defaultbg)
                for each_label in current_objects
    ...
    def pick_word(self):
        ...
        if word_index < len(words_list):
            ...
            self.delineate_cells(char_index)
            [
                self.selected_indexes.update({each_key: a})
                for each_key in self.selected_indexes.keys()
            ]

            self.natural_word = (
                natural_order_list[natural_order_list.index(self.natural_word) + 1]
                if self.natural_word != natural_order_list[-1]
                else natural_order_list[0]
            )
...
class QtFormosa(QMainWindow):
    ...
    def sorted_themes(self) -> tuple[str]:
        ...
        themes = sorted(
            [each_directory.stem for each_directory in files_path.glob(r"*.json")]
        )
        default_index = themes.index(self.DEFAULT_THEME)
        default_theme = themes.pop(default_index)
        sorted_themes = tuple([default_theme]) + tuple(
            themes[:default_index] + themes[default_index:]
        )
